...
function App() {
  ...
  const fetchImages = async () => {
    try {
      ...
      console.log('Imagens recebidas da API:', response.data);
  ...
  // Tela de erro ou slideshow de imagens em tela cheia
  if (error || videos.length === 0) {
    // Se há imagens, mostrar slideshow em tela cheia
    if (showImageSlideshow && images.length > 0) {
      return (
        <div className="relative w-full h-screen overflow-hidden bg-black">
          ...
          <div className="w-full h-full">
            <img
              ...
              onError={(e) => {
                ...
                console.error('Arquivo da imagem:', images[currentImageIndex]?.arquivo);
                // Não esconder a imagem, apenas mostrar um placeholder
                e.target.style.backgroundColor = '#1f2937';
                e.target.style.display = 'flex';
                e.target.style.alignItems = 'center';
                e.target.style.justifyContent = 'center';
                e.target.innerHTML = '<div style="color: white; text-align: center;"><div style="font-size: 4rem;">📷</div><div>Erro ao carregar imagem</div></div>';
              }}
              onLoad={() => {
                console.log('Imagem carregada com sucesso:', images[currentImageIndex]?.titulo);
              }}
            />
          </div>
            ...
            <div className="flex items-end justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-3">
                  <span className="inline-block px-3 py-1 text-sm text-white bg-purple-600 rounded-full">
                    📸 Imagem
                  </span>
                </div>
              </div>
          ...
          <div className="absolute z-20 space-y-2 top-4 right-4">
            ...
            {messages.length > 0 && (
              <div className="px-3 py-1 rounded-full bg-blue-600/80">
                <div className="text-sm text-white">
    ...
    // Se não há imagens, mostrar tela de erro tradicional
    return (
